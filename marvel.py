...
def auth(marvel_private_key, marvel_public_key):
        ...
        hash_string = hashlib.md5("%s%s%s" % (ts, marvel_private_key, marvel_public_key)).hexdigest()
        return "ts=%s&apikey=%s&hash=%s" % (ts, marvel_public_key, hash_string)

api_key = auth(marvel_private_key, marvel_public_key)
request = Request('http://gateway.marvel.com:80/v1/public/characters?offset=' + str(offset) + '&limit=5&' + api_key)

try:
    data1 = json.load(urlopen(request))
